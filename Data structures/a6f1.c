...
void createAlphabetSet (typos_synolou s);
boolean  isVowel(char c);
void  createVowelSet(typos_synolou s);
void  vowelCount (char b,typos_synolou s,int num);
...
void  createVowelSet(typos_synolou s)
{   int i;
    char A [megisto_plithos]="ABCDEFGHIJKLMNOPQRSTUVWXYZ";


    for(i=0; i<megisto_plithos; i+=3)
    {
      if(isVowel(A[i])) Eisagogi(i,s);
    }
