...
{
   int choice;
   char ch,c[26]="ABCDEFGHIJKLMNOPQRSTUVWXYZ",name[20],con;
   HashListType HList;
   ListElm AnItem;
   int AKey,i,j;
   int Loc, Pred;
                   ...
                   for(j=0; j<26; j++)
                    {
                        if(name[i]==c[j])
                        {
                            AnItem.RecKey=AnItem.RecKey+(i+1)*(j+1);
                            break;
                        }
                    }
          }
...
*/
{
	...
	while(index < VMax-1)
	{
		...
		HList->List[index].Data.amount=0;
