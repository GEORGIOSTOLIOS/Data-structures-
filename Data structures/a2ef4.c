...
void swap(ListPointer *List,ListPointer PredPtr);
...
int main () {
   ...
   int number,i;
   ListPointer List,PredPtr=NULL,NextPtr=List->Next,tempptr;
...
swap(&List,PredPtr);
 ...
 LinkedTraverse(List);
   ...
   void swap(ListPointer *List,ListPointer PredPtr)
   {    ListPointer CurrPtr,NextPtr,Outptr,List1;
        ListElementType temp,x,y;
        boolean Found;
   int i;
...
CurrPtr = *List;
         ...
         while ( CurrPtr!=NULL )
        {
            LinkedInsert(&List1,CurrPtr->Data,PredPtr);
